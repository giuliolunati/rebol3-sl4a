 ...
 *
 * Modified by Giulio Lunati (giuliolunati@gmail.com)
 */
public class Rebol3Installer extends AsyncTask<Void, Void, Boolean> {
  ...
  private final Runnable mTaskStarter = new Runnable() {
    ...
    public void run() {
      ...
      try {
        ...
        if (newTask != null) mTaskHolder = newTask.execute();
      } catch (Exception e) {
  ...
  protected boolean setup() {
    File target = new File(InterpreterConstants.SCRIPTS_ROOT);
    if (!installAssets("scripts",target,0644)) return false;
    target = new File(mContext.getFilesDir(),"bin");
    if (!installAssets("bin",target,0755)) return false;
    target = new File(mContext.getFilesDir(),"lib");
    if (!installAssets("lib",target,0644)) return false;
    return true;
  }

  private boolean installAssets(String dir, File target, int mode) {
    AssetManager assetManager = mContext.getAssets();
    target.mkdirs();
    Log.v("installAsset:"+target.getAbsolutePath());
    String[] files = null;
    try {
